#!/usr/bin/env python3
"""Full dataset comparison with all mapped columns."""
...
def full_comparison():
    """Compare full datasets using all mapped columns."""
    ...
    # All mapped columns with data type casting
    df1_clean = df1.select([
        ...
        pl.col("DOB").cast(pl.Utf8).alias("DOB"),
        pl.col("AgeCalc").cast(pl.Utf8).alias("AgeCalc"),
        ...
        pl.col("StreetAddress").cast(pl.Utf8).alias("StreetAddress"),
    ...
    df2_clean = df2.select([
        ...
        pl.col("surname").cast(pl.Utf8).alias("Surname"),
        pl.col("gender").cast(pl.Utf8).alias("Gender"),
        pl.col("dob_yyyymmdd").cast(pl.Utf8).alias("DOB"),
        pl.col("age_band").cast(pl.Utf8).alias("AgeCalc"),
        pl.col("landline_text").cast(pl.Utf8).alias("Landline"),
        pl.col("mobile_text").cast(pl.Utf8).alias("Mobile"),
        pl.col("email").cast(pl.Utf8).alias("EmailStd"),
        pl.col("gnaf_address_label").cast(pl.Utf8).alias("StreetAddress"),
        pl.col("suburb").cast(pl.Utf8).alias("Suburb"),
        pl.col("state").cast(pl.Utf8).alias("State"),
        pl.col("postcode").cast(pl.Utf8).alias("Postcode")
    ])
    ...
    report = sv.compare([df1_pd, "DATADIRECT_DL_Full"], [df2_pd, "ad_consumers_Full"])
    
    output_path = Path("reports/html")
    output_path.mkdir(parents=True, exist_ok=True)
    
    report_file = output_path / "full_mapped_comparison.html"
    report.show_html(str(report_file))
    logger.info(f"Full comparison report saved to: {report_file}")
