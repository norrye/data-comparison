#!/usr/bin/env python3
"""
Comprehensive matching analysis v5.0 - Using updated parquet with landline_text field.

Version: 5.0
Author: Expert Data Scientist
Description: Comprehensive data matching analysis using the updated ad_consumers file with landline_text field
"""
...
def comprehensive_anti_join_analysis_v5() -> None:
    """
    Perform comprehensive anti-join analysis using updated parquet file.
    
    Uses the new ad_consumers_2020805_inter_ll_text.parquet file with landline_text field.
    """
    logger.info("Starting comprehensive anti-join analysis v5.0")
    ...
    logger.info(f"Using files: {file1.name} and {file2.name}")
    ...
    try:
        ...
        print("\n=== COMPREHENSIVE ANTI-JOIN ANALYSIS V5.0 ===")
        print(f"DataDirect: {dd_count:,} records")
        print(f"Ad_consumers: {ac_count:,} records")
        
        # Semantic field mappings from column_mapping_analyzer.py:
        # Title->title, FirstName->given_name_1, Surname->surname, Gender->gender
        # Landline->landline_text, Mobile->mobile_text, EmailStd->email
        # Suburb->suburb, State->state, Postcode->postcode_text
        ...
        for match_name, condition, filter_cond in direct_matches:
            try:
                ...
                # Anti-join 2: AC ANTI JOIN DD (records only in AC)
        ...
        # Concatenated field matches with proper type casting for Postcode (BIGINT -> VARCHAR)
        ...
        # All fields concatenated with proper type casting and semantic mappings
        ...
        conn.close()
        logger.info("Comprehensive anti-join analysis v5.0 completed successfully")
...
if __name__ == "__main__":
    comprehensive_anti_join_analysis_v5()