#!/usr/bin/env python3
"""
Comprehensive matching analysis v9.0 - Using DuckDB native ANTI JOIN syntax.

Version: 9.0
Author: Expert Data Scientist
Description: Uses DuckDB's native ANTI JOIN functionality as requested
"""
...
def comprehensive_anti_join_analysis_v9() -> None:
    """
    Perform comprehensive anti-join analysis using DuckDB native ANTI JOIN syntax.
    
    Uses semantic mappings and DuckDB's ANTI JOIN as specifically requested.
    """
    logger.info("Starting comprehensive anti-join analysis v9.0")
    ...
    try:
        ...
        print("\n=== COMPREHENSIVE ANTI-JOIN ANALYSIS V9.0 ===")
        ...
        logger.info("Processing direct field matches using DuckDB ANTI JOIN")
        ...
        for match_name, condition, filter_cond in direct_matches:
            try:
                ...
                dd_only = conn.execute(f"""
                    SELECT COUNT(*) FROM (
                        SELECT * FROM read_parquet('{file1}') WHERE {dd_filter}
                    ) d
                ...
                ac_only = conn.execute(f"""
                    SELECT COUNT(*) FROM (
                        SELECT * FROM read_parquet('{file2}') WHERE {ac_filter}
                    ) a
        ...
        # Concatenated field matches using ANTI JOIN
        ...
        logger.info("Processing concatenated field matches using DuckDB ANTI JOIN")
        ...
        # All fields concatenated using ANTI JOIN
        logger.info("Processing all fields concatenated using DuckDB ANTI JOIN")
        ...
        conn.close()
        logger.info("Comprehensive anti-join analysis v9.0 completed successfully")
...
if __name__ == "__main__":
    comprehensive_anti_join_analysis_v9()