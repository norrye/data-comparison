#!/usr/bin/env python3
"""Full dataset comparison with corrected mappings."""
...
def full_comparison_corrected():
    """Compare full datasets using corrected column mappings."""
    ...
    # Corrected mapped columns with mobile_text and landline instead of mobile/landline
    ...
    df2_clean = df2.select([
        ...
        pl.col("surname").cast(pl.Utf8).alias("Surname"),
        pl.col("gender").cast(pl.Utf8).alias("Gender"),
        pl.col("landline_text").cast(pl.Utf8).alias("Landline"),  # Using landline (not landline_text)
    ...
    report_file = output_path / "full_corrected_comparison.html"
    report.show_html(str(report_file))
    logger.info(f"Full corrected comparison saved to: {report_file}")

if __name__ == "__main__":
    full_comparison_corrected()