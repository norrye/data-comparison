#!/usr/bin/env python3
"""Final working full dataset comparison with correct columns."""
...
def final_working_comparison():
    """Compare full datasets with actually existing columns."""
    ...
    df2_clean = df2.select([
        ...
        pl.col("surname").cast(pl.Utf8).alias("Surname"),
        pl.col("gender").cast(pl.Utf8).alias("Gender"),
        pl.col("landline").cast(pl.Utf8).alias("Landline"),  # Using landline (not landline_text)
    ...
    report_file = output_path / "final_working_comparison.html"
    report.show_html(str(report_file))
    logger.info(f"Final working comparison saved to: {report_file}")

if __name__ == "__main__":
    final_working_comparison()